...
It combines dense retrieval, neural reranking, source-aware multi-document reasoning, calculator tool use, evidence display, and systematic evaluation.
...
- **Gradio UI** with:
  - final answer on the left
  - supporting evidence and source information on the right
...
ResearchPilot uses a **78-question golden dataset** containing:
...
The retrieval evaluator measures whether the final reranked Top-10 contains the required golden evidence.
...
The complete RAG pipeline was evaluated on the same 78-question dataset.

Each generated answer was scored on:

- **Correctness** — agreement with the expected answer
- **Groundedness** — whether factual claims were supported by retrieved context
- **Completeness** — whether the important parts of the expected answer were covered
...
One example was especially useful for diagnosis:
...
This was one of the most important findings from the project:
...
```text
Which company reported the higher FY2026 EPS?
```

or:

```text
How does Infosys compare with the other companies?
```

the system keeps multi-company evidence.
...
The tool is used for calculations such as:
...
The local Chroma vector database is intentionally excluded from Git tracking.
...
- The vector database is distributed separately rather than rebuilt automatically from the source corpus.
...
The retrieval layer uses direct Chroma queries to retain explicit control over:

- documents
- metadata
- distances
- batched query embeddings

This also keeps the retrieval path simple and transparent.

### Why a reranker?

Dense retrieval at larger `K` substantially improved evidence coverage.

That showed that relevant passages often existed in the candidate pool but needed stronger relevance scoring before being sent to the LLM.
